...
class MixLayoutViewController: BaseViewController {
    ...
    let button1 = UIButton.withText(" < Back ", color: .green)
    let titleLabel = UILabel.withText("Mixed Layout", color: .red)
    let textLabel1 = UILabel.withText("This is an example of vertical box where child views  have additional layouting:\nTitle is placed in center with minimum padding from both sides.\nBoth text label has differnt padding from both sides.", color: .lightGray)
    let textLabel2 = UILabel.withText("Last stack item is another boxView with horizontal layout containing two buttons.\nAny of these views can be skiped in boxView.items, without affecting other views.", color: .lightGray)
    ...
    override func viewDidLoad() {
        ...
        let button2 = UIButton.withText(" Next > ", color: .green)
